...
class AztecToolbar : FrameLayout, IAztecToolbar, OnMenuItemClickListener {
    ...
    private var bgColorMenu: PopupMenu? = null
    private var colorMenu: PopupMenu? = null
...
//                    findViewById<ToggleButton>(ToolbarAction.CODE.buttonId).performClick()
    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        ...
        when (item?.itemId) {
            // Color Menu
            R.id.colorBlack, R.id.colorDarkGreen, R.id.colorBlue, R.id.colorRed,
            R.id.colorOrange, R.id.colorPurple, R.id.colorYellow, R.id.colorWhite -> {
                //aztecToolbarListener?.onToolbarFormatButtonClicked(AztecTextFormat.FORMAT_PARAGRAPH, false)
                var textColor = resources.getColor(R.color.fg_black)
                when (item?.itemId) {
                    R.id.colorDarkGreen -> {
                        textColor = resources.getColor(R.color.fg_darkgreen)
                    }
                    R.id.colorBlue -> {
                        textColor = resources.getColor(R.color.fg_blue)
                    }
                    R.id.colorRed -> {
                        textColor = resources.getColor(R.color.fg_red)
                    }
                    R.id.colorOrange -> {
                        textColor = resources.getColor(R.color.fg_orange)
                    }
                    R.id.colorPurple -> {
                        textColor = resources.getColor(R.color.fg_purple)
                    }
                    R.id.colorYellow -> {
                        textColor = resources.getColor(R.color.fg_yellow)
                    }
                    R.id.colorWhite -> {
                        textColor = resources.getColor(R.color.white)
                    }
                }
                editor?.setSpanColor(textColor)
                editor?.toggleFormatting(AztecTextFormat.FORMAT_COLOR)
                return true
            }
            // Background Menu
            R.id.bgColorYellow, R.id.bgColorGreen, R.id.bgColorBlue,
            R.id.bgColorPink, R.id.bgColorRed, R.id.bgColorPurple, R.id.bgColorNone -> {
                //aztecToolbarListener?.onToolbarFormatButtonClicked(AztecTextFormat.FORMAT_PARAGRAPH, false)
               var bgColor = resources.getColor(R.color.bg_none)
                when (item?.itemId) {
                    R.id.bgColorYellow -> {
                        bgColor = resources.getColor(R.color.bg_yellow)
                    }
                    R.id.bgColorGreen -> {
                        bgColor = resources.getColor(R.color.bg_green)
                    }
                    R.id.bgColorBlue -> {
                        bgColor = resources.getColor(R.color.bg_blue)
                    }
                    R.id.bgColorPink -> {
                        bgColor = resources.getColor(R.color.bg_pink)
                    }
                    R.id.bgColorRed -> {
                        bgColor = resources.getColor(R.color.bg_red)
                    }
                    R.id.bgColorPurple -> {
                        bgColor = resources.getColor(R.color.bg_purple)
                    }
                }
                editor?.setBackgroundSpanColor(bgColor)
                editor?.toggleFormatting(AztecTextFormat.FORMAT_BACKGROUND)
                return true
            }
            // Heading Menu
            R.id.paragraph -> {
...
//            }
    ...
    private fun initView(attrs: AttributeSet?) {
        ...
        for (toolbarAction in ToolbarAction.values()) {
            ...
            if (toolbarAction == ToolbarAction.BACKGROUND) {
                setBgColorMenu(findViewById(toolbarAction.buttonId))
            }

            if (toolbarAction == ToolbarAction.TEXTCOLOR) {
                setColorMenu(findViewById(toolbarAction.buttonId))
            }
    ...
    private fun onToolbarAction(action: IToolbarAction) {
        ...
        if(action == ToolbarAction.BACKGROUND) {
            aztecToolbarListener?.onToolbarBgColorButtonClicked()
            //bgColorMenuHelper?.show()
            bgPopupWindow?.showAtLocation(bgToolView,Gravity.BOTTOM or Gravity.RIGHT ,0,0);
            return
        } else if(action == ToolbarAction.TEXTCOLOR) {
            aztecToolbarListener?.onToolbarColorButtonClicked()
            //colorMenuHelper?.show()
            fgPopupWindow?.showAtLocation(bgToolView,Gravity.BOTTOM or Gravity.RIGHT ,0,0);
            return
        }
...
//        else if (headingMenu?.menu?.findItem(R.id.preformat)?.isChecked!!) return AztecTextFormat.FORMAT_PREFORMAT
    ...
    /*private var bgColorMenuHelper: MenuPopupHelper? = null
    private fun setBgColorMenu(view: View) {
        bgColorMenu = PopupMenu(context, view)
        bgColorMenu?.setOnMenuItemClickListener(this)
        bgColorMenu?.inflate(R.menu.bg_color_menu)

        bgColorMenuHelper = MenuPopupHelper(context, bgColorMenu?.menu as MenuBuilder, view)
        bgColorMenuHelper?.setForceShowIcon(true)
    }*/

    private var bgPopupWindow: PopupWindow? = null
    private var bgToolView: View? = null
    private fun setBgColorMenu(view: View) {
        bgToolView = view
        val inflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
        val popupView: View = inflater.inflate(R.layout.bg_popup_window, null)

        val width = LinearLayout.LayoutParams.WRAP_CONTENT
        val height = LinearLayout.LayoutParams.WRAP_CONTENT
        val focusable = true
        bgPopupWindow = PopupWindow(popupView, width, height, focusable)

        val imgY: AppCompatImageView = popupView.findViewById(R.id.bgImgYellow)
        imgY.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_yellow))
            applyBgColor()
        }

        val imgG: AppCompatImageView = popupView.findViewById(R.id.bgImgGreen)
        imgG.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_green))
            applyBgColor()
        }

        val imgB: AppCompatImageView = popupView.findViewById(R.id.bgImgBlue)
        imgB.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_blue))
            applyBgColor()
        }

        val imgP: AppCompatImageView = popupView.findViewById(R.id.bgImgPink)
        imgP.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_pink))
            applyBgColor()
        }

        val imgR: AppCompatImageView = popupView.findViewById(R.id.bgImgRed)
        imgR.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_red))
            applyBgColor()
        }

        val imgPu: AppCompatImageView = popupView.findViewById(R.id.bgImgPurple)
        imgPu.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_purple))
            applyBgColor()
        }

        val imgNone: AppCompatImageView = popupView.findViewById(R.id.bgImgNone)
        imgNone.setOnClickListener {
            editor?.setBackgroundSpanColor(resources.getColor(R.color.bg_none))
            applyBgColor()
        }
   ...
   /* private var colorMenuHelper: MenuPopupHelper? = null
    @SuppressLint("RestrictedApi")
    private fun setColorMenu(view: View) {
        colorMenu = PopupMenu(context, view)
        colorMenu?.setOnMenuItemClickListener(this)
        colorMenu?.inflate(R.menu.color_menu)

        colorMenuHelper = MenuPopupHelper(context, colorMenu?.menu as MenuBuilder, view)
        colorMenuHelper?.setForceShowIcon(true)
    }*/

    private var fgPopupWindow: PopupWindow? = null
    private var fgToolView: View? = null
    private fun setColorMenu(view: View) {
        fgToolView = view
        val inflater = context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
        val popupView: View = inflater.inflate(R.layout.fg_popup_window, null)

        val width = LinearLayout.LayoutParams.WRAP_CONTENT
        val height = LinearLayout.LayoutParams.WRAP_CONTENT
        val focusable = true
        fgPopupWindow = PopupWindow(popupView, width, height, focusable)

        val imgB: AppCompatImageView = popupView.findViewById(R.id.fgImgBlack)
        imgB.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_black))
            applyFgColor()
        }

        val imgW: AppCompatImageView = popupView.findViewById(R.id.fgImgWhite)
        imgW.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.white))
            applyFgColor()
        }

        val imgR: AppCompatImageView = popupView.findViewById(R.id.fgImgRed)
        imgR.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_red))
            applyFgColor()
        }

        val imgO: AppCompatImageView = popupView.findViewById(R.id.fgImgOrange)
        imgO.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_orange))
            applyFgColor()
        }

        val imgY: AppCompatImageView = popupView.findViewById(R.id.fgImgYellow)
        imgY.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_yellow))
            applyFgColor()
        }

        val imgD: AppCompatImageView = popupView.findViewById(R.id.fgImgDark)
        imgD.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_darkgreen))
            applyFgColor()
        }

        val imgBu: AppCompatImageView = popupView.findViewById(R.id.fgImgBlue)
        imgBu.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_blue))
            applyFgColor()
        }

        val imgP: AppCompatImageView = popupView.findViewById(R.id.fgImgPurple)
        imgP.setOnClickListener {
            editor?.setSpanColor(resources.getColor(R.color.fg_purple))
            applyFgColor()
        }
    }

    private fun applyFgColor(){
        editor?.toggleFormatting(AztecTextFormat.FORMAT_COLOR)
        fgPopupWindow?.dismiss()
    }
